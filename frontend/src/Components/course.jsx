...
const Course = () => {
    ...
    async function fetchCourse() {
      try {
        const response = await axios.get(
          `http://localhost:8087/api/courses/${courseId}`
        );
  ...
  const handleDuration = () => {
    ...
    if(duration!=0){
      fetch("http://localhost:8087/api/progress/update-duration", {
        method: "PUT",
        headers: {
  ...
  useEffect(() => {
    fetch(`http://localhost:8087/api/progress/${userId}/${courseId}`)
  ...
  useEffect(() => {
    const updateProgress = async () => {
        if (courseId && userId) {
            try {
                const response = await fetch("http://localhost:8087/api/progress/update-progress", {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
                    },

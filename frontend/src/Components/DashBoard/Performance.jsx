...
const Performance = () => {
  ...
  useEffect(() => {
    async function fetchCourse() {
      try {
        ...
        const response = await axios.get(`http://localhost:8087/api/learning/${userId}`);
  ...
  useEffect(()=>{
    const userId = localStorage.getItem("id");
    fetch(`http://localhost:8087/api/assessments/perfomance/${userId}`).then((res)=>res.json()).then((data)=>{setPerfomanceData(data)});
